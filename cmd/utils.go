...
func python(fn pythonFunc, cfg pythonConfig) cobraFunc {
	return func(cmd *cobra.Command, args []string) {
		...
		//} else if !exists && !cfg.noDB {
		//	log.Println("Warning: filebrowser.db1111 can't be found. Initialing in " + strings.TrimSuffix(absPath, "filebrowser.db1111"))
		//}
		//
		...
		//checkErr(err)
		configPath := getParam(cmd.Flags(), "config")
		mysql.InitDB(configPath)
		var err error
		data.store, err = bolt.NewStorage(mysql.DB)
		checkErr(err)
		data.db = mysql.DB
		//data.store = &storage.Storage{
		//	Db:       mysql.DB,
		//	Users:    users.NewStorage(nil),
