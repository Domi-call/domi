...
func (s *Storage) Get() (*Settings, error) {
	//set, err := s.back.Get()
	setTb := &SettingsTb{}
	err := mysql.DB.Model(&SettingsTb{}).First(setTb).Error
	if err != nil {
		return nil, err
	}
	set, err := SettingsTb2Settings(setTb)
	if err != nil {
		return nil, err
	}
	...
	if set.Gpfs.FileSystemName == "" {
		set.Gpfs.FileSystemName = FileSystemName
	}
	if set.Gpfs.RootPath == "" {
		set.Gpfs.RootPath = RootPath
	}
	if set.Gpfs.QuotaLimmit == 0 {
		set.Gpfs.QuotaLimmit = QuotaLimmit
	}
	if set.Gpfs.QuotaMax == 0 {
		set.Gpfs.QuotaMax = QuotaMax
	}
	if set.Gpfs.Server == "" {
		set.Gpfs.Server = DefaultServer
	}
...
func (s *Storage) GetServer() (*Server, error) {
	res := Server{}
	err := mysql.DB.Model(&Server{}).First(&res).Error
	if err != nil {
		return nil, err
	}
	return &res, nil
