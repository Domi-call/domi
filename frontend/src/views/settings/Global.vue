<template>
  ...
  <div class="row" v-else-if="!layoutStore.loading && settings !== null">
    ...
    <div class="column">
      <form class="card" @submit.prevent="save">
        <div class="card-title">
          <h2>{{ t("settings.gpfsGuiApi") }}</h2>
        </div>
        <div class="card-content">
          <div>
            <p>
              <label for="Path">{{ t("settings.gpfsGuiApiPath") }}</label>
              <input
                class="input input--block"
                type="text"
                v-model="settings.gpfs.api"
              />
            </p>
            <p>
              <label for="gpfsUser">{{ t("settings.gpfsUser") }}</label>
              <input
                class="input input--block"
                type="text"
                v-model="settings.gpfs.username"
              />
            </p>
            <p>
              <label for="gpfsPassword">{{ t("settings.gpfsPassword") }}</label>
              <input
                class="input input--block"
                type="password"
                v-model="settings.gpfs.password"
              />
            </p>
            <p>
              <label for="fileSystemName">{{
                t("settings.fileSystemName")
              }}</label>
              <input
                class="input input--block"
                type="text"
                v-model="settings.gpfs.fileSystemName"
              />
            </p>
            <p>
              <label for="rootPath">{{ t("settings.rootPath") }}</label>
              <input
                class="input input--block"
                type="text"
                v-model="settings.gpfs.rootPath"
              />
            </p>
            <p>
              <label for="quotaLimmit">{{ t("settings.quotaLimmit") }}</label>
              <input
                class="input input--block"
                type="number"
                v-model="settings.gpfs.quotaLimmit"
              />
            </p>
            <p>
              <label for="quotaMax">{{ t("settings.quotaMax") }}</label>
              <input
                class="input input--block"
                type="number"
                v-model="settings.gpfs.quotaMax"
              />
            </p>
            <p>
              <label for="server">{{ t("settings.server") }}</label>
              <input
                class="input input--block"
                type="text"
                v-model="settings.gpfs.server"
              />
            </p>
            <p>
              <label for="sysuser">{{ t("settings.sysuser") }}</label>
              <input
                class="input input--block"
                type="text"
                v-model="settings.gpfs.sysuser"
              />
            </p>
            <p>
              <label for="privateKey">{{ t("settings.privateKey") }}</label>
              <textarea
                rows="5"
                style="width: 100%"
                v-model="settings.gpfs.privateKey"
              />
            </p>
          </div>
        </div>
